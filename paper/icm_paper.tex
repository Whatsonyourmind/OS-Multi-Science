...
% ============================================================
\begin{abstract}
When multiple models---spanning statistical methods, machine learning, and simulation---are applied to the same prediction task, practitioners face a recurring question: do these models agree because of independent evidence, or is their apparent consensus an artifact of shared biases or correlated training data? Existing ensemble diversity measures each capture only a single facet of inter-model agreement and offer no mechanism to distinguish genuine from spurious convergence. We introduce the \emph{Index of Convergence Multi-epistemic} (ICM), a five-component diagnostic score that aggregates distributional agreement, directional consensus, uncertainty overlap, perturbation invariance, and a dependency penalty into a single bounded value in $(0,1)$ via logistic aggregation. We prove that ICM is bounded, monotone, and Lipschitz continuous, and establish a formal connection to prediction risk: under a testable calibration condition, thresholding on ICM monotonically controls the ensemble error rate, and high ICM implies vanishing ambiguity in the Krogh-Vedelsby decomposition. We couple ICM with a conformal risk control (CRC) gating mechanism that provides distribution-free coverage guarantees, partitioning predictions into ACT, DEFER, and AUDIT decisions. Experiments on eleven datasets with eight model families plus a deep ensemble baseline, evaluated via stratified 5-fold cross-validation, show that ICM-weighted predictions achieve competitive accuracy (mean $0.953 \pm 0.03$), valid conformal coverage (mean 0.984), and error detection comparable to entropy baselines (Spearman $\rho = 0.184$ vs.\ $0.178$). A wide-range preset expands the effective score range by $16.1\times$, enabling three-way decision gating with ACT rates that automatically calibrate from 14\% on a 40-class face recognition task to 83\% on easy datasets. Ablation analysis confirms that every component contributes, and the framework is robust across weight presets. The value of ICM lies not in surpassing baselines on raw accuracy, but in providing a decomposable, formally grounded diagnostic signal that reveals \emph{when} and \emph{why} an ensemble's output should be trusted.
\end{abstract}
...
\begin{itemize}
    ...
    \item \textbf{Predictive entropy}~\citep{lakshminarayanan2017simple}: Captures spread in the predictive distribution but conflates aleatoric and epistemic uncertainty~\citep{depeweg2018decomposition} and provides no dependency correction.
    ...
    \item \textbf{Selective prediction}~\citep{geifman2017selective,elyanov2010foundations}: Learns when to abstain from prediction based on model confidence, providing a principled reject option. ICM's three-way ACT/DEFER/AUDIT gate generalizes this to multi-model convergence with conformal coverage guarantees.
\end{itemize}

\paragraph{Contributions.} We propose the Index of Convergence Multi-epistemic (ICM), a unified framework that addresses these limitations. The contribution is not any single component metric---each has precedents in the literature---but rather the identification of \emph{which} dimensions are jointly necessary and sufficient for reliable convergence diagnosis, the formal analysis of their composition, and the coupling with distribution-free decision gating. Just as the ambiguity decomposition~\citep{krogh1994neural} derives lasting value not from squared error (a known quantity) but from identifying the correct decomposition and proving its properties, ICM's value lies in the specific five-dimensional decomposition and the guarantees it affords. To our knowledge, no prior diagnostic combines distributional agreement, directional consensus, uncertainty overlap, perturbation invariance, and a dependency penalty into a single bounded, Lipschitz-continuous score. Concretely, the framework makes five contributions:
...
\begin{enumerate}
    ...
    \item \textbf{Formal guarantees}: boundedness (Proposition~\ref{prop:bounded}), monotonicity in each positive component (Proposition~\ref{prop:monotone}), Lipschitz continuity with constant $L = (\text{scale}/4)\|\bw^*\|$ (Proposition~\ref{prop:lipschitz}), and a formal connection between ICM and prediction risk---error-rate monotonicity under calibration (Proposition~\ref{prop:risk_monotone}) and a link to the Krogh-Vedelsby ambiguity decomposition (Proposition~\ref{prop:ambiguity_link}).
    \item A \textbf{conformal risk control (CRC) gating} mechanism that maps ICM scores to calibrated risk bounds with distribution-free coverage guarantees, partitioning predictions into ACT/DEFER/AUDIT decisions (Theorem~\ref{thm:coverage}). Conformal prediction has been applied to single-model nonconformity scores~\citep{vovk2005algorithmic}; applying it to multi-model convergence diagnostics with a three-way decision gate is, to our knowledge, new.
    \item \textbf{Anti-saturation mechanisms}---adaptive normalization, argmax-based direction, and perturbation scaling---that expand the effective score range substantially, enabling practical decision gating.
    \item \textbf{Comprehensive benchmarking} on eleven datasets (nine classification, two regression) with eight model families plus a deep ensemble baseline, including ablation studies, dependency penalty activation, gating comparisons against simpler uncertainty methods, and weight sensitivity analysis.
\end{enumerate}
...
\paragraph{Selective prediction and learning to defer.} The three-way decision gate relates to the classical reject option~\citep{chow1970optimum} and the selective prediction framework of~\citet{elyanov2010foundations}. Recent work on learning to defer~\citep{mozannar2020consistent} learns when to delegate to a human expert. Our approach differs in using an unsupervised convergence signal (ICM) rather than a supervised deferral policy, and in providing conformal coverage guarantees. The gating architecture also shares structural similarities with mixture-of-experts systems~\citep{jacobs1991adaptive,shazeer2017outrageously}, though ICM gates at the prediction level rather than routing inputs to specialized sub-networks.

\paragraph{Why five components?} The five ICM components were chosen to span the key orthogonal dimensions of ensemble agreement: distributional overlap ($A$), prediction direction ($D$), uncertainty calibration ($U$), stability ($C$), and independence ($\Pi$). Together they capture what no single metric can: the agreement component measures \emph{if} models agree, the direction component measures \emph{where} they agree, the uncertainty component measures \emph{how confidently} they agree, the invariance component measures \emph{how stably} they agree, and the dependency component measures \emph{whether} that agreement is genuine.
...
\begin{remark}[Discontinuity in $D$ under category changes]
The direction score $D$ depends on the cardinality $|\mathcal{C}|$ of distinct observed signs through $H_{\max} = \log(|\mathcal{C}|)$. When a new sign category appears, $H_{\max}$ increases discontinuously, causing a jump in $D$. In classification with argmax mode (used in all our experiments), direction is computed from per-sample class-vote entropy, which varies continuously with the predictive distributions and avoids this discontinuity.
\end{remark}
...
Motivated by the observation that small input perturbations can dramatically change neural network predictions~\citep{szegedy2014intriguing}, we measure ensemble stability under noise. Given pre-perturbation predictions $\hat{y}^{\mathrm{pre}}$ and post-perturbation predictions $\hat{y}^{\mathrm{post}}$:
\begin{equation}
...
\begin{remark}[Adaptive $C_A$ and monotonicity]
When the adaptive normalization constant $C_A$ is used (computed from the 90th percentile of observed pairwise distances), the mapping from individual model outputs to the agreement component $A$ is not globally monotone, because changing one model's output can shift $C_A$ and thereby affect $A$ for all pairs. Proposition~\ref{prop:monotone} guarantees monotonicity of $\icm$ in $A$ as a real-valued input; the composition with adaptive $C_A$ introduces a local non-monotonicity that we observe to be negligible in practice, since the 90th percentile is stable under small perturbations.
\end{remark}
...
\subsection{ICM as a Predictor of Ensemble Error}

The preceding propositions establish regularity properties of the ICM function itself. A natural and more consequential question is whether ICM scores carry information about \emph{prediction risk}: does a higher ICM score imply that the ensemble prediction is more likely to be correct? We formalize this connection through two results. Proposition~\ref{prop:risk_monotone} establishes that, under a calibration condition, ICM induces a stochastic ordering on the conditional error rate. Proposition~\ref{prop:ambiguity_link} connects the ICM agreement component to the classical ambiguity decomposition of ensemble error.

\begin{proposition}[Error-Rate Monotonicity Under Calibration]\label{prop:risk_monotone}
Let $\hat{y}$ denote the ensemble prediction (e.g., majority vote or ICM-weighted average) and let $E = \ind[\hat{y} \neq y]$ be the error indicator. Suppose the ICM score is \emph{calibrated} in the following sense: the conditional correctness probability $p(c) \coloneqq \PP(\hat{y} = y \mid \icm = c)$ is non-decreasing in $c$. Then for any thresholds $\tau' \leq \tau$:
\begin{equation}\label{eq:risk_monotone}
\PP(E = 1 \mid \icm \geq \tau) \leq \PP(E = 1 \mid \icm \geq \tau')
\end{equation}
That is, restricting to higher-ICM samples yields a lower (or equal) error rate.
\end{proposition}

\begin{proof}
Define $q(c) = 1 - p(c) = \PP(E = 1 \mid \icm = c)$, which is non-increasing by assumption. For any threshold $\tau$, the conditional error rate over the set $\{\icm \geq \tau\}$ is:
\[
\PP(E = 1 \mid \icm \geq \tau) = \frac{\int_\tau^1 q(c)\, dF(c)}{\int_\tau^1 dF(c)}
\]
where $F$ is the CDF of the ICM score. Since $q$ is non-increasing, this weighted average is non-increasing in $\tau$: raising the threshold removes the lowest-ICM (highest-error-rate) mass from the average. Formally, for $\tau \geq \tau'$, the support $[\tau, 1] \subseteq [\tau', 1]$ and $q(c) \leq q(c')$ for $c \geq c'$, so the conditional mean of $q$ over $[\tau, 1]$ is at most that over $[\tau', 1]$.
\end{proof}

\begin{remark}[Calibration assumption]
The calibration condition---that $\PP(\text{correct} \mid \icm = c)$ is non-decreasing---is an empirical property, not a tautology. It asserts that the composite ICM score, which aggregates distributional agreement, directional consensus, uncertainty overlap, and perturbation invariance, is indeed aligned with correctness. Our experiments support this: Experiment~3 shows positive Spearman correlations between inverted ICM and per-sample loss across all datasets where errors occur (mean $\rho = 0.184$), and the CRC isotonic regression in Experiment~2 successfully learns a monotone risk function from ICM scores with valid coverage (mean 0.984). The calibration assumption can be tested on any new dataset via the isotonic regression step of the CRC pipeline.
\end{remark}

\begin{proposition}[Link to Ambiguity Decomposition]\label{prop:ambiguity_link}
Consider a regression ensemble with ICM-weighted predictions $\hat{y} = \sum_{k=1}^K \omega_k \hat{y}_k$ where $\omega_k \propto \icm_k$ and $\sum_k \omega_k = 1$. By the Krogh-Vedelsby decomposition~\citep{krogh1994neural}, the squared ensemble error decomposes as:
\begin{equation}\label{eq:ambiguity}
(\hat{y} - y)^2 = \underbrace{\sum_k \omega_k (\hat{y}_k - y)^2}_{\bar{E}\;\text{(weighted avg.\ individual error)}} - \underbrace{\sum_k \omega_k (\hat{y}_k - \hat{y})^2}_{\bar{A}\;\text{(ambiguity)}}
\end{equation}
The ambiguity term $\bar{A}$ is the weighted variance of individual predictions around the ensemble mean. When the agreement component $A$ of ICM is high (small pairwise distributional distances), the individual predictions $\hat{y}_k$ are tightly clustered, so $\bar{A}$ is small. Combined with the ICM weighting, which assigns higher weight $\omega_k$ to models with higher per-model ICM (thereby down-weighting outlier models), this implies:
\begin{equation}
\text{High } \icm \;\Longrightarrow\; \text{small } \bar{A} \;\Longrightarrow\; (\hat{y} - y)^2 \approx \bar{E}
\end{equation}
Thus, under high convergence the ensemble error is governed almost entirely by the average individual model quality, and the ambiguity penalty vanishes. Conversely, when $\icm$ is low, disagreement inflates $\bar{A}$, and the decomposition~\eqref{eq:ambiguity} alone does not bound $(\hat{y} - y)^2$---hence the ensemble output should not be trusted without further inspection.
\end{proposition}

\begin{proof}
The decomposition~\eqref{eq:ambiguity} is algebraic and holds for any weighted combination~\citep{krogh1994neural}. It remains to show that high $A$ implies small $\bar{A}$. By Definition~\ref{def:agreement}, $A$ is close to 1 when the mean pairwise distributional distance $\bar{d}$ is small relative to $C_A$. For point predictions, $\bar{d}$ small implies $\hat{y}_k \approx \hat{y}_j$ for most pairs, so $\hat{y}_k \approx \hat{y}$ for all $k$, and therefore $\bar{A} = \sum_k \omega_k(\hat{y}_k - \hat{y})^2$ is small. More precisely, if $\max_k |\hat{y}_k - \hat{y}| \leq \delta$, then $\bar{A} \leq \delta^2$. Since $\bar{d} \leq C_A(1 - A)$, and $\bar{d}$ upper-bounds $\max_k |\hat{y}_k - \hat{y}|$ up to a constant depending on $K$ and the distance metric, we obtain $\bar{A} = O((1 - A)^2)$ as $A \to 1$.
\end{proof}

\begin{remark}[Interpretation and limitations]
Proposition~\ref{prop:risk_monotone} shows that ICM provides a valid \emph{risk ordering}: filtering by ICM threshold monotonically controls the error rate, provided the calibration condition holds. This is the property exploited by the CRC gating system (Section~\ref{sec:crc}). Proposition~\ref{prop:ambiguity_link} provides the mechanistic link: high ICM implies low ambiguity, so the ensemble error is determined by individual model quality rather than disagreement. Neither result claims that ICM \emph{causes} accuracy; rather, ICM serves as a sufficient statistic for ranking predictions by expected error, which is the operationally relevant property for decision gating.
\end{remark}
...
\paragraph{Exp 9: Weight sensitivity.} Sweep component weights across seven presets (default, equal, four single-component-dominant, and penalty-heavy) to assess robustness. Report accuracy, score range, and ACT rate stability.
...
\begin{table}[t]
\centering
\caption{Classification accuracy by method across seven representative datasets. ICM-Weighted uses per-sample ICM scores to weight model contributions. Bold indicates best per dataset. ICM-Weighted is best on 2/9 classification datasets (including ties).}
\label{tab:prediction}
\begin{tabular}{lccccccc}
\toprule
Method & Iris & Wine & Breast C. & Digits & Moons & Circles & Olivetti \\
\midrule
ICM-Weighted       & 0.933 & 0.972 & 0.956 & 0.983 & \textbf{0.985} & \textbf{0.995} & 0.888 \\
Ensemble Avg       & 0.933 & \textbf{1.000} & 0.965 & 0.975 & \textbf{0.985} & \textbf{0.995} & 0.900 \\
Stacking (LR)      & \textbf{0.967} & \textbf{1.000} & \textbf{0.974} & 0.981 & \textbf{0.985} & \textbf{0.995} & 0.888 \\
Stacking (RF)      & \textbf{0.967} & 0.972 & 0.947 & 0.981 & \textbf{0.985} & 0.985 & 0.900 \\
Deep Ensemble      & 0.833 & \textbf{1.000} & 0.956 & \textbf{0.986} & 0.980 & 0.990 & \textbf{0.938} \\
\midrule
\multicolumn{8}{l}{\emph{Mean ICM-Weighted accuracy (all 9 datasets, 5-fold CV): 0.953; Mean best baseline: 0.964}} \\
\bottomrule
\end{tabular}
\end{table}

Under 5-fold cross-validation, ICM-Weighted predictions are competitive with baselines across all nine classification tasks (Table~\ref{tab:prediction}). ICM-Weighted is the top method on 2 of 9 classification datasets (including ties). The Deep Ensemble baseline uses five architecturally diverse MLPs with varied hidden layer sizes, activation functions, and learning rates to maximize disagreement-based diversity. On the most challenging dataset (Covertype, 7 classes, 54 features), all methods struggle: ICM-Weighted achieves 0.750 vs.\ 0.755 for the best baseline. On Olivetti Faces (40 classes), ICM-Weighted achieves 0.888, while the Deep Ensemble achieves 0.938 due to its architectural diversity. On the synthetic Moons and Circles datasets, most methods achieve near-perfect accuracy ($\geq 0.985$), confirming that the model zoo handles nonlinear boundaries well.
...
ICM-CRC gating achieves a mean coverage of 0.984 across all nine classification datasets under 5-fold CV, exceeding the nominal $1-\alpha = 0.90$ level (Table~\ref{tab:uq}). We note that this over-coverage (98.4\% at a 90\% target) indicates that the current CRC calibration is conservative: prediction sets are larger than strictly necessary to achieve the nominal level. This conservatism stems from using a single isotonic regression model across all samples; adaptive or class-conditional calibration could tighten the sets while maintaining valid coverage. The average prediction set size is 1.40---on most samples, the CRC system identifies one or two classes. The largest set sizes occur on Olivetti Faces (mean 2.22) and Circles (mean 1.63), reflecting higher inherent uncertainty. Split conformal prediction achieves coverage closer to the nominal level (mean 0.96) with slightly smaller sets (mean 1.01), demonstrating the coverage-efficiency trade-off between the two approaches.
...
\begin{table}[t]
\centering
\caption{Mean ICM components across all nine classification datasets (wide-range preset). All components vary across datasets---none are saturated at 0 or 1. Olivetti Faces (40 classes) shows low agreement ($A{=}0.245$) and uncertainty overlap ($U{=}0.275$), driving its low overall ICM score.}
...
Table~\ref{tab:range} illustrates the importance of the anti-saturation mechanisms. The default logistic configuration compresses all scores into a narrow band of width $0.022$--$0.081$ (mean range $0.051$), making three-way decision gating impractical. The wide-range preset expands the range to $0.566$--$0.921$ (mean range $0.781$), enabling ACT/DEFER/AUDIT partitioning.
...
Table~\ref{tab:dependency} addresses a limitation noted in Section~\ref{sec:discussion}: when residual correlation data is provided, the dependency penalty activates. Even with diverse model families, residual correlations from shared training data yield $\Pi \approx 0.52$, reducing the ICM score from 0.635 to 0.473. When we deliberately construct a correlated ensemble (multiple random forests with different seeds), $\Pi$ increases to 0.63 and the ICM score drops to 0.352, correctly reflecting reduced epistemic diversity. This demonstrates that ICM's dependency penalty distinguishes genuine from spurious convergence when the necessary information is available.
...
Table~\ref{tab:gating} compares ICM-based gating against simpler alternatives. ICM provides a different operating point: it routes 61.7\% of predictions to ACT with 98.1\% accuracy while flagging 11.9\% for AUDIT. Entropy-only gating is more conservative (40.4\% ACT, perfect accuracy, 8.2\% AUDIT), while max-probability confidence is more aggressive (76.9\% ACT, 98.6\% accuracy, only 1.9\% AUDIT). The ICM's AUDIT rate of 11.9\%---substantially higher than the alternatives---reflects its multi-dimensional assessment: samples flagged for AUDIT have low agreement, fragmented direction, \emph{and} poor uncertainty overlap, making them genuinely uncertain rather than merely having low entropy. This higher AUDIT rate is a feature, not a bug: in safety-critical settings, identifying the samples that warrant human review is as important as automating confident predictions.

\subsection{Weight Sensitivity (Exp 9)}

To assess whether ICM's diagnostic value depends critically on a specific weight configuration, we sweep across seven weight presets spanning a broad region of the weight simplex, including four single-component-dominant configurations.

\begin{table}[t]
\centering
\caption{Weight sensitivity analysis across seven presets. Accuracy, mean ICM score range, and ACT rate are averaged over nine classification datasets. Accuracy varies by less than 0.5 percentage points across all presets, confirming robustness to weight choice.}
\label{tab:weight_sensitivity}
\begin{tabular}{lccccc|ccc}
\toprule
Preset & $w_A$ & $w_D$ & $w_U$ & $w_C$ & $\lambda$ & Accuracy & Score Range & ACT\% \\
\midrule
Default         & 0.35 & 0.15 & 0.25 & 0.10 & 0.15 & 0.950 & 0.77 & 61.7 \\
Equal           & 0.20 & 0.20 & 0.20 & 0.20 & 0.20 & 0.947 & 0.72 & 60.2 \\
$A$-dominant    & 0.50 & 0.15 & 0.15 & 0.10 & 0.10 & 0.947 & 0.80 & 65.1 \\
$D$-dominant    & 0.15 & 0.50 & 0.15 & 0.10 & 0.10 & 0.946 & 0.91 & 61.2 \\
$U$-dominant    & 0.15 & 0.15 & 0.50 & 0.10 & 0.10 & 0.948 & 0.68 & 74.4 \\
$C$-dominant    & 0.15 & 0.15 & 0.10 & 0.50 & 0.10 & 0.951 & 0.34 & 92.7 \\
$\Pi$-dominant  & 0.10 & 0.10 & 0.10 & 0.10 & 0.60 & 0.947 & 0.17 & 0.0 \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:weight_sensitivity} shows that the framework is robust to weight selection. Across all seven presets---spanning equal weighting, single-component dominance, and heavy penalty configurations---the mean classification accuracy varies by less than 0.5 percentage points (0.946--0.951). The core presets (Default through $U$-dominant) maintain ACT rates in a 60--74\% band. The two extreme presets reveal expected boundary behavior: $C$-dominant achieves 92.7\% ACT because the invariance component $C \approx 0.92$ is near-constant across samples, while $\Pi$-dominant collapses to 0\% ACT because the heavy penalty pushes all scores below the gating threshold (in our benchmark, $\Pi = 0$ so there is nothing to offset the large $\lambda$). These extremes are informative---they confirm that the $C$ component contributes stability signal while the $\Pi$ penalty requires residual data to be useful---but the five non-extreme presets demonstrate that the framework's diagnostic value is robust to the specific weight vector chosen.
...
ICM is not designed to surpass baselines on raw prediction accuracy---ensemble averaging and stacking serve that purpose. Rather, ICM provides a decomposable diagnostic signal that answers the question: \emph{why} do models agree or disagree on this prediction?
...
\paragraph{Formal connection to prediction risk.}
Beyond its diagnostic role, ICM has a formal relationship to prediction error. Proposition~\ref{prop:risk_monotone} establishes that if ICM is calibrated---i.e., the conditional probability of correctness is non-decreasing in the ICM score---then thresholding on ICM monotonically controls the error rate. This is not a vacuous condition: our CRC pipeline (Section~\ref{sec:crc}) explicitly fits an isotonic regression from ICM to loss, enforcing this monotone relationship, and Experiment~3 empirically confirms the correlation between ICM and error across nine datasets. Proposition~\ref{prop:ambiguity_link} provides the mechanistic underpinning via the Krogh-Vedelsby decomposition: high ICM (specifically, high agreement $A$) implies low ambiguity in the ensemble, so the prediction error converges to the average individual model error. Together, these results establish ICM not merely as a descriptive summary of agreement, but as a score with provable implications for when ensemble predictions can be trusted.

\paragraph{Difficulty-calibrated decision gating.}
The benchmark reveals that ICM's decision gating automatically scales with problem difficulty---a property we term \emph{difficulty calibration}. As shown in Table~\ref{tab:decision}, ACT rates range from 14\% on the hardest task to 93\% on the easiest, without any dataset-specific threshold tuning. This calibration emerges from the five-component decomposition: when agreement ($A$), directional consensus ($D$), and uncertainty overlap ($U$) are jointly low, the composite ICM score drops, triggering conservative gating. No single baseline metric provides this multi-dimensional difficulty signal.
...
\begin{enumerate}
    ...
    \item \textbf{Invariance component near-constant.} The perturbation invariance $C$ shows low variance across datasets (0.903--0.932), indicating it provides limited discriminative signal in our benchmark. This is because all models in our zoo produce stable predictions under small input perturbations on tabular data. In domains with adversarially sensitive models or high-dimensional inputs (e.g., images, text), we expect $C$ to vary more and contribute greater diagnostic value.

    \item \textbf{Conservative coverage.} The ICM-CRC system achieves 98.4\% coverage at a nominal 90\% target, indicating over-conservative prediction sets. This results from using a single isotonic calibration model. Adaptive or class-conditional calibration could tighten sets closer to the nominal level without sacrificing the coverage guarantee.
...
\paragraph{Relationship to Bayesian model averaging.}
ICM and BMA~\citep{hoeting1999bayesian} serve different purposes. BMA weights models by posterior probability to optimize predictive accuracy, requiring a likelihood specification. ICM diagnoses the \emph{nature} of agreement by decomposing geometric properties of model outputs, requiring no likelihood. The two are complementary: BMA could serve as the aggregation method while ICM provides the diagnostic layer that reveals \emph{why} the posterior concentrates on certain models. We do not include BMA as a prediction baseline because ICM is not designed to compete on raw accuracy; rather, it augments any aggregation method with interpretable convergence diagnostics.

\paragraph{Computational cost.}
The ICM computation is dominated by pairwise distance calculations ($O(K^2 C)$ for Hellinger with $C$ classes) and the Ledoit-Wolf correlation ($O(K^2 N)$). For typical settings ($K = 3$--$8$, $C \leq 40$, $N \leq 10^4$), the total runtime is under one second on commodity hardware. The full benchmark suite (11 datasets, 8 models plus deep ensemble, 9 experiments) completes in approximately 140 seconds.

\paragraph{On the nature of the contribution.}
One might ask whether composing known metrics via a weighted sigmoid constitutes a genuine contribution. We argue that the contribution is analogous to the ambiguity decomposition~\citep{krogh1994neural}: the individual ingredients (squared error, model predictions) were well known, but identifying the correct decomposition and proving its properties yielded lasting theoretical and practical value. Similarly, ICM identifies five specific dimensions---distributional agreement, directional consensus, uncertainty overlap, perturbation invariance, and dependency penalty---that jointly characterize multi-model convergence, proves formal properties (boundedness, monotonicity, Lipschitz continuity) of their composition, and couples the result with conformal risk control for distribution-free decision gating. The sigmoid is not the contribution; it is a convenient bounded activation. The contributions are: (i) the selection of which dimensions are necessary and sufficient for convergence diagnosis, supported by the ablation analysis in Section~\ref{sec:results}; (ii) the formal guarantees on the composed score, which do not follow trivially from properties of the individual components; and (iii) the CRC gating mechanism that converts convergence scores into calibrated three-way decisions with coverage guarantees---an application of conformal prediction to multi-model diagnostics rather than to single-model nonconformity scores. This combination is not available in prior work on ensemble diversity metrics.
...
We have presented the Index of Convergence Multi-epistemic (ICM), a five-component framework for diagnosing the nature and strength of agreement among epistemically diverse models. The ICM score is provably bounded, monotone, and Lipschitz continuous; under calibration it induces a monotone ordering on prediction error rates (Proposition~\ref{prop:risk_monotone}); and it can be converted to calibrated risk bounds via conformal risk control gating with distribution-free coverage guarantees.

Experiments on eleven datasets demonstrate that ICM provides competitive accuracy (mean $0.953 \pm 0.03$), valid conformal coverage (mean 0.984), and error detection comparable to entropy baselines. The wide-range preset expands the effective score range by $16.1\times$, enabling three-way decision gating with ACT rates that automatically calibrate to problem difficulty. Ablation analysis confirms that every component contributes, and the dependency penalty correctly distinguishes genuine from spurious convergence when residual data is provided.

The contribution is \emph{diagnostic}: ICM decomposes the opaque concept of ``ensemble agreement'' into five interpretable, formally grounded components, each providing distinct information about why models agree or disagree. This decomposition is particularly relevant to safety-critical applications where knowing \emph{when to trust} an ensemble matters as much as the ensemble's prediction itself.

\paragraph{Future work.} Directions include: (1) evaluation on domain-specific multi-paradigm ensembles in climate science, epidemiology, and finance; (2) conditional coverage extensions via weighted conformal prediction; (3) online ICM updates for streaming prediction settings; and (4) tightening the ICM-risk connection (Propositions~\ref{prop:risk_monotone}--\ref{prop:ambiguity_link}) to obtain finite-sample error bounds under explicit distributional assumptions.
