...
% ============================================================
\begin{abstract}
When multiple models---spanning statistical methods, machine learning, and simulation---are applied to the same prediction task, practitioners face a fundamental question: do these models \emph{truly} agree, or is their apparent consensus an artifact of shared biases, correlated training data, or saturated metrics? Existing ensemble diversity measures such as the Q-statistic, pairwise disagreement, and predictive entropy each capture only a single facet of inter-model agreement and offer no mechanism to distinguish genuine from spurious convergence. We introduce the \emph{Index of Convergence Multi-epistemic} (ICM), a five-component diagnostic score that aggregates distributional agreement, directional consensus, uncertainty overlap, perturbation invariance, and a dependency penalty into a single bounded value in $(0,1)$ via logistic aggregation. We prove that the ICM is bounded, monotone in each positive component, and Lipschitz continuous with a small constant ($L \approx 0.12$). We couple ICM with a conformal risk control (CRC) gating mechanism that provides distribution-free coverage guarantees and partitions predictions into ACT, DEFER, and AUDIT decisions. Experiments on eleven datasets (nine classification, two regression) with eight diverse model families plus a deep ensemble baseline show that ICM-weighted predictions achieve competitive accuracy (mean 0.9462 across classification tasks), the ICM-CRC system provides valid conformal coverage (mean 0.9910) with tight prediction sets (mean size 1.44), and ICM-based error detection matches entropy baselines (Spearman $\rho = 0.2359$ vs.\ $0.2443$). The wide-range preset expands the effective score range by $16.1\times$ over default logistic aggregation, enabling meaningful three-way decision gating. Crucially, ICM automatically calibrates ACT rates to problem difficulty---from 83\% on easy datasets to 13\% on a 40-class face recognition task---with all five components varying meaningfully across the evaluation suite ($A{=}0.443$, $D{=}0.753$, $U{=}0.791$, $C{=}0.921$). The primary value of ICM is not in surpassing baselines on raw accuracy, but in providing a decomposable, formally grounded diagnostic signal that reveals \emph{when} and \emph{why} an ensemble's output should be trusted.
\end{abstract}
...
\begin{enumerate}
    ...
    \item \textbf{Formal guarantees}: boundedness (Proposition~\ref{prop:bounded}), monotonicity in each positive component (Proposition~\ref{prop:monotone}), and Lipschitz continuity with constant $L \approx 0.12$ (Proposition~\ref{prop:lipschitz}).
    ...
    \item \textbf{Comprehensive benchmarking} on eleven datasets (nine classification, two regression) with eight model families plus a deep ensemble baseline, demonstrating competitive prediction quality, valid uncertainty quantification, effective error detection, and difficulty-calibrated decision gating.
\end{enumerate}
...
We define the \emph{linear pre-activation} as $z = w_A A + w_D D + w_U U + w_C C - \lambda \Pi$, so that in the default configuration $\icm = \sigma(z)$.
...
For all valid inputs where $A, D, U, C, \Pi \in [0,1]$ and all weights $w_A, w_D, w_U, w_C, \lambda > 0$:
\begin{equation}
\icm(\cF) \in (0, 1)
\end{equation}
More precisely, $\sigma(-\lambda) \leq \icm \leq \sigma(w_A + w_D + w_U + w_C)$. With default parameters: $\icm \in [\sigma(-0.15), \sigma(0.85)] \approx [0.463, 0.701]$.
\end{proposition}

\begin{proof}
The pre-activation $z = w_A A + w_D D + w_U U + w_C C - \lambda \Pi$ achieves its minimum $z_{\min} = -\lambda$ when $A = D = U = C = 0$ and $\Pi = 1$, and its maximum $z_{\max} = w_A + w_D + w_U + w_C$ when $A = D = U = C = 1$ and $\Pi = 0$. Since $\sigma: \RR \to (0,1)$ is strictly increasing, $\sigma(z_{\min}) \leq \icm \leq \sigma(z_{\max})$.
\end{proof}

\begin{proposition}[Monotonicity]\label{prop:monotone}
The ICM is strictly monotone increasing in each of $A$, $D$, $U$, $C$ and strictly monotone decreasing in $\Pi$:
\begin{equation}
\frac{\partial\,\icm}{\partial A} = w_A \sigma'(z) > 0, \quad \frac{\partial\,\icm}{\partial \Pi} = -\lambda \sigma'(z) < 0
\end{equation}
where $\sigma'(z) = \sigma(z)(1 - \sigma(z)) > 0$.
\end{proposition}

\begin{proof}
By the chain rule, $\partial \icm / \partial A = \sigma'(z) \cdot w_A$. Since $\sigma'(z) > 0$ for all $z \in \RR$ and $w_A > 0$, the derivative is strictly positive. The argument for $D$, $U$, $C$ is identical. For $\Pi$: $\partial \icm / \partial \Pi = \sigma'(z) \cdot (-\lambda) < 0$.
\end{proof}

\begin{proposition}[Lipschitz Continuity]\label{prop:lipschitz}
For any two component vectors $\bc = (A, D, U, C, \Pi)$ and $\bc' = (A', D', U', C', \Pi')$:
\begin{equation}
|\icm(\bc) - \icm(\bc')| \leq L_{\icm} \cdot \|\bc - \bc'\|, \quad L_{\icm} = \frac{1}{4}\sqrt{w_A^2 + w_D^2 + w_U^2 + w_C^2 + \lambda^2}
\end{equation}
With default parameters: $L_{\icm} = \frac{1}{4}\sqrt{0.2400} \approx 0.122$.
\end{proposition}

\begin{proof}
The sigmoid satisfies $\sup_z |\sigma'(z)| = 1/4$, so $\sigma$ is $\frac{1}{4}$-Lipschitz. The pre-activation is linear with coefficient vector $\bw^* = (w_A, w_D, w_U, w_C, -\lambda)$. By Cauchy-Schwarz, $|z(\bc) - z(\bc')| \leq \|\bw^*\| \cdot \|\bc - \bc'\|$. Composing: $|\icm(\bc) - \icm(\bc')| \leq \frac{1}{4}\|\bw^*\| \cdot \|\bc - \bc'\|$.
\end{proof}

\begin{remark}
The small Lipschitz constant ($\approx 0.12$) means that the sigmoid aggregation acts as a smoothing operator: a perturbation of magnitude $\epsilon$ in the component vector changes the ICM by at most $0.12\epsilon$. This is beneficial for stability but compresses the output range, motivating the wide-range preset which uses $\text{scale}=10$ to amplify sensitivity.
\end{remark}
...
\begin{itemize}
    ...
    \item \textbf{Bootstrap Ensemble:} Bagged ensemble with bootstrap resampling.
...
\paragraph{Exp 2: Uncertainty quantification.} Compare ICM-CRC gating against split conformal prediction on marginal coverage, average prediction set size, and conditional coverage standard deviation.
...
Bootstrap Ens.     & 0.933 & \textbf{1.000} & 0.965 & 0.975 & \textbf{0.985} & \textbf{0.995} & 0.938 \\
Deep Ensemble      & 0.867 & 0.972 & 0.947 & 0.978 & 0.965 & \textbf{0.995} & 0.938 \\
\midrule
\multicolumn{8}{l}{\emph{Mean ICM-Weighted accuracy (all 9 datasets): 0.9462; Mean best baseline: 0.9673}} \\
\bottomrule
\end{tabular}
\end{table}

ICM-Weighted predictions achieve a mean accuracy of 0.9462 across all nine classification tasks, compared to 0.9673 for the best baseline per dataset. ICM-Weighted is the top method on 3 of 9 classification datasets (Digits, Moons tied, Circles tied). On the most challenging dataset (Covertype, 7 classes, 54 features), all methods struggle: ICM-Weighted achieves 0.770 vs.\ 0.848 for Stacking-LR. On Olivetti Faces (40 classes), ICM-Weighted achieves 0.938, close to the best Stacking-RF at 0.963. On the synthetic Moons and Circles datasets, most methods achieve near-perfect accuracy ($\geq 0.985$), confirming that the model zoo handles nonlinear boundaries well.

For regression, ICM-Weighted achieves RMSE 0.530 on California Housing (vs.\ 0.580 for ensemble average) and RMSE 51.78 on Diabetes (vs.\ 51.02 for ensemble average), showing mixed results.
...
ICM-CRC gating achieves a mean coverage of 0.9910 across all nine classification datasets, exceeding the nominal $1-\alpha = 0.90$ level (Table~\ref{tab:uq}). The average prediction set size is 1.44, indicating tight uncertainty quantification---on most samples, the CRC system identifies one or two classes with high confidence. The largest set sizes occur on Olivetti Faces (mean 2.29) and Circles (mean 1.68), reflecting the higher inherent uncertainty in those tasks. Split conformal prediction achieves comparable coverage with slightly smaller sets. The coverage advantage of ICM-CRC comes at the cost of marginally larger sets, reflecting its conservative approach to uncertainty.
...
Table~\ref{tab:error} shows that ICM-based error detection (using inverted ICM scores as an uncertainty signal) achieves a mean Spearman correlation of 0.2359 across all nine classification datasets, comparable to ensemble entropy at 0.2443 and exceeding bootstrap disagreement at 0.2143. On the Iris dataset, ICM matches entropy at $\rho = 0.417$ while far outperforming bootstrap disagreement ($\rho = 0.073$). The strongest error detection is observed on Olivetti Faces ($\rho = 0.384$) and Covertype ($\rho = 0.343$), the two most challenging datasets. On the Wine dataset, all methods achieve $\rho = 0$ because no errors were made by the ensemble on the test set.
...
Table~\ref{tab:components} reveals that all four positive components vary meaningfully across datasets. Agreement ($A$) ranges from 0.245 (Olivetti Faces) to 0.573 (Iris), reflecting genuine differences in how well eight diverse model families agree across datasets of varying difficulty. Directional consensus ($D$) ranges from 0.456 (Olivetti, 40 classes) to 0.922 (Moons, clean 2-class boundary). Uncertainty overlap ($U$) shows the widest variation: from 0.275 (Olivetti) to 0.998 (Breast Cancer). Invariance ($C$) is consistently high (0.903--0.936), indicating stable predictions under small perturbations across all datasets. The dependency penalty $\Pi = 0$ across all datasets because the benchmark uses genuinely independent model families trained on the same features---without residual correlation or gradient similarity information provided.
...
\midrule
Olivetti Faces & 40 & 12.5\% & 46.2\% & 41.2\% \\
Circles        & 2  & 32.5\% & 66.0\% & 1.5\%  \\
Covertype      & 7  & 45.8\% & 54.0\% & 0.2\%  \\
Digits         & 10 & 68.9\% & 23.6\% & 7.5\%  \\
Iris           & 3  & 80.0\% & 20.0\% & 0.0\%  \\
Wine           & 3  & 80.6\% & 13.9\% & 5.6\%  \\
Breast Cancer  & 2  & 80.7\% & 5.3\%  & 14.0\% \\
Moons          & 2  & 82.5\% & 7.5\%  & 10.0\% \\
Concept Drift  & 3  & 83.3\% & 8.3\%  & 8.3\%  \\
\bottomrule
\end{tabular}
\end{table}

The decision gate breakdown reveals the \textbf{key finding} of the expanded evaluation: ICM automatically calibrates ACT rates to problem difficulty without any dataset-specific tuning. Easy datasets with clean boundaries and few classes (Moons, Concept Drift, Iris, Wine, Breast Cancer) receive 80--83\% ACT rates. Moderately difficult tasks (Digits with 10 classes, Covertype with 7 classes and 54 features) receive 46--69\% ACT rates with substantial DEFER allocations. The hardest task---Olivetti Faces with 40 classes and only 10 samples per class---receives just 13\% ACT with 41\% AUDIT, correctly reflecting that the ensemble has low confidence on most predictions. This difficulty-calibrated gating emerges automatically from the five-component decomposition: low agreement ($A{=}0.245$), low directional consensus ($D{=}0.456$), and low uncertainty overlap ($U{=}0.275$) on Olivetti Faces jointly drive the ICM score down, whereas high values on all three components for easy datasets drive scores up. No single uncertainty metric (entropy, disagreement, or max-probability) captures this multi-faceted difficulty calibration.
...
\begin{enumerate}
    \item \textbf{Component weights are domain-dependent.} The default weights ($w_A = 0.35$, $w_D = 0.15$, $w_U = 0.25$, $w_C = 0.10$, $\lambda = 0.15$) were set based on domain knowledge, not learned from data. Our meta-learner framework can optimize these weights, but optimal weights may differ across application domains.
    ...
    \item \textbf{Dependency penalty activation.} In our benchmarks, $\Pi = 0$ because the model zoo uses independent families trained on shared features without providing residual or gradient information. Full activation of the dependency penalty requires model residuals, feature provenance metadata, and gradient attributions, which are available in production settings but not in standard benchmark setups.
...
Our experiments on eleven datasets demonstrate that ICM provides a competitive weighting signal (mean accuracy 0.9462 across nine classification tasks), valid uncertainty quantification (coverage 0.9910 with tight sets of size 1.44), and effective error detection (Spearman $\rho = 0.2359$, comparable to entropy at 0.2443). The wide-range preset expands the effective score range by $16.1\times$, enabling meaningful three-way decision gating with ACT rates that automatically calibrate from 13\% on the hardest task (40-class face recognition) to 83\% on easy tasks---a difficulty-calibrated response that emerges from the five-component decomposition.

The fundamental contribution is \emph{diagnostic}: ICM decomposes the opaque concept of ``ensemble agreement'' into five interpretable, formally grounded components---distributional agreement, directional consensus, uncertainty overlap, perturbation invariance, and dependency---each providing distinct information about why models agree or disagree. The expanded benchmark confirms that these components vary meaningfully ($A{=}0.443$, $D{=}0.753$, $U{=}0.791$, $C{=}0.921$) and jointly produce difficulty-calibrated gating across tasks ranging from simple 2-class problems to complex 40-class recognition. We believe this decomposition will be particularly valuable in safety-critical applications where knowing \emph{when to trust} an ensemble matters as much as the ensemble's prediction itself.
...
$\frac{\partial\,\icm}{\partial \Pi} = -\lambda \cdot \sigma'(z) < 0$.
The maximum sensitivity is $\lambda/4 = 0.0375$ (with default parameters), attained at $z = 0$.
\end{proposition}

\begin{proof}
Direct computation via chain rule. $\sigma'(z) = \sigma(z)(1-\sigma(z)) > 0$ and $\lambda > 0$.
\end{proof}
...
We conducted a Wilcoxon signed-rank test comparing ICM-Weighted vs.\ Ensemble Average accuracy across the nine classification datasets: statistic $= 1.0$, $p = 0.9375$. There is no statistically significant difference, confirming that ICM-Weighted is competitive but not superior on raw accuracy. Similarly, comparing ICM vs.\ Entropy error detection: statistic $= 5.0$, $p = 0.1562$---no significant difference.
